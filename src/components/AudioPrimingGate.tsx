...
export default function AudioPrimingGate({ children }: { children: React.ReactNode }) {
  ...
  const handleContinue = async () => {
    ...
    try {
      await unlock();
      // Audio prewarm removed (was custom logic in audioPreload.ts).
      // Browser HTTP cache handles repeat plays naturally — first play
      // pays a small cold-fetch cost, subsequent plays are instant.
      // The custom prewarm was forcing 20 file fetches at this exact
      // moment, competing for bandwidth with the welcome video that
      // starts ~immediately after this gate dismisses.
      setDismissed(true);
    } finally {
      setBusy(false);
    }
        ...
        {!dismissed && (
          ...
          >
            {/* Home page video as the backdrop. Muted so it can autoplay
                before the user grants the audio activation gesture. No preload
